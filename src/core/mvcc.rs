...
mod tests {
    ...
    fn test_create_version() {
        ...
        let version_id = manager.create_version(key.clone(), transaction_id, data.clone()).unwrap();
    ...
    fn test_read_version() {
        ...
        // Создаём версию
        manager.create_version(key.clone(), transaction_id, data.clone()).unwrap();
        ...
        let read_data = manager.read_version(&key, transaction_id, snapshot).unwrap();
    ...
    fn test_vacuum() {
        ...
        manager.create_version(key.clone(), transaction_id, data).unwrap();
        manager.abort_transaction(transaction_id).unwrap();
    ...
    fn test_multiple_versions() {
        ...
        manager.create_version(key.clone(), tx2, data2.clone()).unwrap();
