...
use crate::core::{
    ConcurrencyManager, TransactionId, RowKey, Timestamp, ResourceType,
};
...
async fn test_write_operation_with_timeout() {
    ...
    let key = RowKey { table_id: 1, row_id: 1 };
    ...
    assert!(true); // Тест прошел, если не завис
}
...
async fn test_begin_transaction() {
    ...
    // Начинаем транзакцию
    let result = manager.begin_transaction(tx1, crate::core::concurrency::IsolationLevel::ReadCommitted);
...
async fn test_lock_conflict_with_timeout() {
    ...
    let result = tokio::time::timeout(
        ...
        async {
            ...
            manager.acquire_write_lock(tx1, resource.clone(), None).await?;
            
            // TX2 пытается получить ту же блокировку с коротким таймаутом
            let result = manager.acquire_write_lock(
                tx2, 
                resource, 
