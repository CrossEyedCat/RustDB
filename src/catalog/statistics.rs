...
impl StatisticsManager {
    ...
    pub fn collect_table_statistics(&mut self, table_name: &str) -> Result<TableStatistics> {
        ...
        column_stats.insert("id".to_string(), self.create_column_statistics("id", 10000, 0, 1, 10000));
        column_stats.insert("name".to_string(), self.create_column_statistics("name", 9500, 500, 0, 0));
        column_stats.insert("age".to_string(), self.create_column_statistics("age", 100, 0, 18, 65));
        column_stats.insert("salary".to_string(), self.create_column_statistics("salary", 1000, 0, 30000, 150000));
        ...
        self.table_statistics.insert(table_name.to_string(), table_stats.clone());
        Ok(table_stats)
    }

    /// Создать статистику колонки
    pub fn create_column_statistics(&self, column_name: &str, distinct: usize, nulls: usize, min_val: i64, max_val: i64) -> ColumnStatistics {
        let value_distribution = if column_name == "id" {
            ValueDistribution::Uniform { step: 1.0 }
        } else if column_name == "age" {
            ValueDistribution::Normal { mean: 35.0, std_dev: 10.0 }
        } else {
    ...
    pub fn estimate_selectivity(&self, table_name: &str, column_name: &str, condition: &str) -> Result<f64> {
    ...
    fn calculate_selectivity(&self, column_stats: &ColumnStatistics, condition: &str) -> Result<f64> {
    ...
    pub fn estimate_result_rows(&self, table_name: &str, column_name: &str, condition: &str) -> Result<usize> {
        if let Some(table_stats) = self.get_table_statistics(table_name) {
...
mod tests {
    ...
    fn test_selectivity_estimation() {
        ...
        let selectivity_like = manager.estimate_selectivity("users", "name", "LIKE").unwrap();
        ...
        let selectivity_unknown = manager.estimate_selectivity("users", "id", "UNKNOWN").unwrap();
    ...
    fn test_result_rows_estimation() {
        ...
        let rows_unknown = manager.estimate_result_rows("unknown_table", "id", "=").unwrap();
    ...
    fn test_statistics_settings_update() {
        ...
        assert_eq!(manager.settings().update_interval_seconds, new_settings.update_interval_seconds);
        assert_eq!(manager.settings().max_histogram_buckets, new_settings.max_histogram_buckets);
        assert_eq!(manager.settings().enable_debug_logging, new_settings.enable_debug_logging);
        
        // Проверяем, что настройки отличаются от исходных
        assert_ne!(manager.settings().auto_update, original_settings.auto_update);
        assert_ne!(manager.settings().update_interval_seconds, original_settings.update_interval_seconds);
    }
    ...
    fn test_value_distribution_variants() {
        ...
        let normal = ValueDistribution::Normal { mean: 42.0, std_dev: 5.0 };
        match normal {
            ValueDistribution::Normal { mean, std_dev } => {
                assert_eq!(mean, 42.0);
