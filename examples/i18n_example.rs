...
use rustdb::common::{Language, MessageKey, set_language, t, t_with_params, I18N, I18nManager};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Обнаружена взаимоблокировка: {}", t(MessageKey::DeadlockDetected));
    
    println!("\nСообщения транзакций:");
    println!("Транзакция начата: {}", t(MessageKey::TransactionStarted));
    println!("Транзакция подтверждена: {}", t(MessageKey::TransactionCommitted));
    println!("Транзакция откачена: {}", t(MessageKey::TransactionRolledBack));
    ...
    set_language(Language::English)?;
    println!("Английский: {}", t_with_params(MessageKey::Welcome, &["RustDB"]));
    
    // Пример с русским языком
    set_language(Language::Russian)?;
    println!("Русский: {}", t_with_params(MessageKey::Welcome, &["RustDB"]));
