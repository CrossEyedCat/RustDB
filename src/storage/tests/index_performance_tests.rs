...
impl BenchmarkResult {
    ...
    fn print(&self) {
        println!("  {} - {}: {} элементов за {:?} ({:.0} ops/sec)", 
                self.index_type, self.operation, self.elements, 
                self.duration, self.ops_per_second);
    }
}
...
fn test_insertion_performance() {
    ...
    for &size in &test_sizes {
        ...
        results.push(BenchmarkResult::new("вставка", "B+ дерево", size, btree_duration));
        ...
        results.push(BenchmarkResult::new("вставка", "Хеш-индекс", size, hash_duration));
    }
    ...
    for result in &results {
        assert!(result.ops_per_second > 1000.0, "Слишком медленная вставка: {:.0} ops/sec", result.ops_per_second);
    }
}
...
fn test_search_performance() {
    ...
    results.push(BenchmarkResult::new("последовательный поиск", "B+ дерево", SIZE, btree_sequential));
    ...
    results.push(BenchmarkResult::new("последовательный поиск", "Хеш-индекс", SIZE, hash_sequential));
    ...
    results.push(BenchmarkResult::new("случайный поиск", "B+ дерево", SIZE, btree_random));
    ...
    results.push(BenchmarkResult::new("случайный поиск", "Хеш-индекс", SIZE, hash_random));
    ...
    for result in &results {
        assert!(result.ops_per_second > 10_000.0, "Слишком медленный поиск: {:.0} ops/sec", result.ops_per_second);
    }
}
...
fn test_range_query_performance() {
    ...
    for &range_size in &range_sizes {
        ...
        println!("  Диапазон размером {}: {} результатов за {:?}", 
                range_size, results.len(), duration);
...
fn test_deletion_performance() {
    ...
    results.push(BenchmarkResult::new("удаление", "Хеш-индекс", delete_count, deletion_duration));
...
fn test_mixed_operations_performance() {
    ...
    println!("  Смешанные операции: {} операций за {:?} ({:.0} ops/sec)", 
            OPERATIONS, total_duration, ops_per_second);
    
    assert!(ops_per_second > 50_000.0, "Слишком медленные смешанные операции: {:.0} ops/sec", ops_per_second);
}
...
fn test_memory_usage_scaling() {
    ...
    for &size in &sizes {
        ...
        println!("  B+ дерево с {} элементами: ~{} байт базовой структуры", size, btree_size);
        
        // Тестируем хеш-индекс
        let mut hash_index: SimpleHashIndex<i32, String> = SimpleHashIndex::new();
        for i in 1..=size {
            hash_index.insert(i as i32, format!("value_{:06}", i)).unwrap();
        }
        
        let hash_size = mem::size_of_val(&hash_index);
        println!("  Хеш-индекс с {} элементами: ~{} байт базовой структуры", size, hash_size);
...
fn test_cache_efficiency() {
    ...
    let random_keys: Vec<i32> = (1..=SIZE).map(|i| ((i * 31337) % SIZE + 1) as i32).collect();
    let start = Instant::now();
    for &key in &random_keys {
        let _ = btree.search(&key).unwrap();
    }
