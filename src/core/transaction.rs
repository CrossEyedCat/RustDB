...
impl TransactionManager {
    ...
    pub fn begin_transaction(&self, isolation_level: IsolationLevel, read_only: bool) -> Result<TransactionId> {
        // Проверяем лимит одновременных транзакций
        {
            let active = self.active_transactions.read()
                .map_err(|_| Error::internal("Failed to acquire read lock on active transactions".to_string()))?;
        ...
        // Генерируем новый ID
        let transaction_id = TransactionId(self.next_transaction_id.fetch_add(1, std::sync::atomic::Ordering::SeqCst));
        ...
        // Добавляем в активные транзакции
        {
            let mut active = self.active_transactions.write()
                .map_err(|_| Error::internal("Failed to acquire write lock on active transactions".to_string()))?;
    ...
    pub fn commit_transaction(&self, transaction_id: TransactionId) -> Result<()> {
        ...
        let transaction_info = {
            ...
            active.get(&transaction_id).cloned()
                .ok_or_else(|| Error::TransactionError(format!("Transaction {} not found", transaction_id)))?
        };
        
        // Проверяем состояние транзакции
        if transaction_info.state != TransactionState::Active {
            return Err(Error::TransactionError(
                format!("Cannot commit transaction {} in state {:?}", transaction_id, transaction_info.state)
            ));
        }
    ...
    pub fn get_transaction_info(&self, transaction_id: TransactionId) -> Result<Option<TransactionInfo>> {
        let active = self.active_transactions.read()
            .map_err(|_| Error::internal("Failed to acquire read lock on active transactions".to_string()))?;
    ...
    pub fn acquire_lock(&self, transaction_id: TransactionId, resource: String, lock_type: LockType, lock_mode: LockMode) -> Result<()> {
        // Проверяем, что транзакция активна
        self.ensure_transaction_active(transaction_id)?;
        
        // Пытаемся получить блокировку
        let acquired = self.lock_manager.acquire_lock(transaction_id, resource.clone(), lock_type, lock_mode)?;
        ...
        if acquired {
            ...
            return Err(Error::TransactionError(
                format!("Failed to acquire lock on resource {} for transaction {}", resource, transaction_id)
            ));
        }
    ...
    pub fn release_lock(&self, transaction_id: TransactionId, resource: String) -> Result<()> {
        ...
        self.lock_manager.release_lock(transaction_id, resource.clone())?;
    ...
    fn update_transaction_state(&self, transaction_id: TransactionId, new_state: TransactionState) -> Result<()> {
        let mut active = self.active_transactions.write()
            .map_err(|_| Error::internal("Failed to acquire write lock on active transactions".to_string()))?;
    ...
    fn ensure_transaction_active(&self, transaction_id: TransactionId) -> Result<()> {
        ...
        match active.get(&transaction_id) {
            ...
            Some(info) => Err(Error::TransactionError(
                format!("Transaction {} is not active (state: {:?})", transaction_id, info.state)
            )),
