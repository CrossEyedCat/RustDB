...
use crate::core::{
    ...
    TransactionManagerConfig, LockType, LockMode
};
...
fn test_transaction_lifecycle() {
    ...
    // Начинаем транзакцию
    let txn_id = tm.begin_transaction(IsolationLevel::ReadCommitted, false).unwrap();
...
fn test_transaction_abort() {
    ...
    let txn_id = tm.begin_transaction(IsolationLevel::Serializable, true).unwrap();
...
fn test_multiple_transactions() {
    ...
    for i in 0..5 {
        let read_only = i % 2 == 0;
        let isolation = if i % 2 == 0 { IsolationLevel::ReadCommitted } else { IsolationLevel::Serializable };
...
fn test_transaction_limit() {
    ...
    let txn1 = tm.begin_transaction(IsolationLevel::ReadCommitted, false).unwrap();
    let txn2 = tm.begin_transaction(IsolationLevel::ReadCommitted, false).unwrap();
    ...
    let txn3 = tm.begin_transaction(IsolationLevel::ReadCommitted, false).unwrap();
...
fn test_lock_operations() {
    ...
    tm.acquire_lock(txn_id, resource.clone(), LockType::Table("users".to_string()), LockMode::Shared).unwrap();
...
fn test_concurrent_transactions() {
    ...
    for i in 0..4 {
        ...
        let handle = thread::spawn(move || {
            let txn_id = tm_clone.begin_transaction(IsolationLevel::ReadCommitted, false).unwrap();
            
            // Получаем блокировку на уникальный ресурс
            let resource = format!("resource_{}", i);
            tm_clone.acquire_lock(
                txn_id,
                resource,
...
fn test_read_only_transactions() {
    ...
    let txn_id = tm.begin_transaction(IsolationLevel::ReadCommitted, true).unwrap();
    let info = tm.get_transaction_info(txn_id).unwrap().unwrap();
    assert!(info.read_only);
...
fn test_transaction_manager_statistics() {
    ...
    tm.acquire_lock(txn1, "resource1".to_string(), LockType::Resource("resource1".to_string()), LockMode::Shared).unwrap();
    tm.acquire_lock(txn2, "resource2".to_string(), LockType::Resource("resource2".to_string()), LockMode::Exclusive).unwrap();
