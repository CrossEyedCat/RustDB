...
fn test_indexed_table_basic_operations() {
    ...
    let alice_id = table.insert("Alice".to_string(), 25, "alice@example.com".to_string()).unwrap();
    let bob_id = table.insert("Bob".to_string(), 30, "bob@example.com".to_string()).unwrap();
    let _charlie_id = table.insert("Charlie".to_string(), 25, "charlie@example.com".to_string()).unwrap();
...
fn test_indexed_table_with_duplicates() {
    ...
    let _john1 = table.insert("John".to_string(), 30, "john1@example.com".to_string()).unwrap();
    let _john2 = table.insert("John".to_string(), 30, "john2@example.com".to_string()).unwrap();
    let _john3 = table.insert("John".to_string(), 25, "john3@example.com".to_string()).unwrap();
...
fn test_index_with_page_manager_simulation() {
    ...
    let range_results = page_index.range_search(&"banana".to_string(), &"date".to_string()).unwrap();
    let affected_pages: std::collections::HashSet<u32> = range_results.into_iter().map(|(_, page_id)| page_id).collect();
