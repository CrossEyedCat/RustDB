...
fn lock_operations_demo() {
    ...
    let resources = vec![
        ("users_table", LockType::Table("users".to_string()), LockMode::Shared),
        ("user_email_index", LockType::Index("idx_user_email".to_string()), LockMode::Shared),
        ("user_record_1", LockType::Record(1, 1), LockMode::Exclusive),
        ("temp_resource", LockType::Resource("temp".to_string()), LockMode::Exclusive),
    ];
    
    for (name, lock_type, lock_mode) in &resources {
        tm.acquire_lock(txn_id, name.to_string(), lock_type.clone(), lock_mode.clone()).unwrap();
        println!("   ✓ Получена блокировка {:?} на {}", lock_mode, name);
    }
    
    // Проверка заблокированных ресурсов
    let info = tm.get_transaction_info(txn_id).unwrap().unwrap();
    println!("   ✓ Всего заблокировано ресурсов: {}", info.locked_resources.len());
    
    // Освобождение одной блокировки
    tm.release_lock(txn_id, "temp_resource".to_string()).unwrap();
...
fn isolation_levels_demo() {
    ...
    let levels = vec![
        (IsolationLevel::ReadUncommitted, "Чтение незафиксированных данных"),
        (IsolationLevel::ReadCommitted, "Чтение зафиксированных данных"),
...
fn statistics_demo() {
    ...
    println!("      • Макс. одновременных транзакций: {}", cfg.max_concurrent_transactions);
    println!("      • Таймаут блокировки: {} мс", cfg.lock_timeout_ms);
    println!("      • Обнаружение дедлоков: {}", cfg.enable_deadlock_detection);
    ...
    for i in 0..5 {
        let read_only = i % 2 == 0;
        let txn_id = tm.begin_transaction(IsolationLevel::ReadCommitted, read_only).unwrap();
    ...
    println!("      • Операций разблокирования: {}", stats.unlock_operations);
