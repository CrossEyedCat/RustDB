...
use rustdb::analyzer::{SemanticAnalyzer, SemanticAnalyzerSettings, AnalysisContext};
...
fn main() -> Result<()> {
    ...
    println!("   Проверка существования объектов: {}", settings.check_object_existence);
    println!("   Проверка типов: {}", settings.check_types);
    println!("   Проверка прав доступа: {}", settings.check_access_rights);
    println!("   Кэширование метаданных: {}", settings.enable_metadata_cache);
    ...
    println!("   Проверено объектов: {}", result.statistics.objects_checked);
    println!("   Проверок типов: {}", result.statistics.type_checks);
    println!("   Время анализа: {} мс", result.statistics.analysis_time_ms);
    ...
    let result = analyzer.analyze(&statement, &context)?;
    println!("   Запрос валиден: {}", result.is_valid);
    println!("   Типы результата: {} колонок", result.type_info.result_types.len());
    ...
    println!("\n4. Анализ INSERT запроса:");
    let mut parser = SqlParser::new("INSERT INTO users (name, email, age) VALUES ('Alice', 'alice@example.com', 25)")?;
    let statement = parser.parse()?;
    ...
    for (i, result) in results.iter().enumerate() {
        println!("     Запрос {}: валиден={}, ошибок={}, предупреждений={}", 
                i + 1, result.is_valid, result.errors.len(), result.warnings.len());
    }
    ...
    println!("   Проверка объектов: {}", updated_settings.check_object_existence);
    println!("   Кэширование: {}", updated_settings.enable_metadata_cache);
    println!("   Строгая валидация: {}", updated_settings.strict_validation);

    // 11. Очистка кэша
    println!("\n11. Очистка кэша анализатора:");
    analyzer.clear_cache();
    let (hits_after, misses_after) = analyzer.cache_statistics();
    println!("   Кэш очищен. Попадания: {}, промахи: {}", hits_after, misses_after);
