...
fn bench_file_manager_operations(c: &mut Criterion) {
    ...
    group.bench_function("write_block", |b| {
        b.iter(|| {
            ...
            file_manager.write_block(black_box(file_id), black_box(1), black_box(&block_data)).unwrap();
        });
    });
...
fn bench_extension_strategies(c: &mut Criterion) {
    ...
    for (name, strategy) in strategies {
        group.bench_with_input(BenchmarkId::new("create_file", name), &strategy, |b, strategy| {
            b.iter(|| {
                let mut manager = AdvancedFileManager::new(temp_path.clone()).unwrap();
                let file_id = manager.create_database_file(
                    "strategy_test.db",
