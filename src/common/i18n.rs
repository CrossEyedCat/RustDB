...
impl I18nManager {
    ...
    fn load_messages(&mut self) {
        ...
        self.messages
            .insert(Language::Russian, Self::russian_messages());
    }
    ...
    fn russian_messages() -> LocalizedMessages {
        ...
        messages.insert(
            ...
            "Ошибка подключения".to_string(),
        );
        messages.insert(
            MessageKey::TransactionError,
            "Ошибка транзакции".to_string(),
        );
        messages.insert(MessageKey::LockTimeout, "Таймаут блокировки".to_string());
        messages.insert(
            MessageKey::DeadlockDetected,
            "Обнаружена взаимоблокировка".to_string(),
        );
        ...
        messages.insert(
            MessageKey::ConstraintViolation,
            "Нарушение ограничения".to_string(),
        );
        ...
        messages.insert(
            MessageKey::TooManyConnections,
            "Слишком много подключений".to_string(),
        );
        ...
        messages.insert(
            MessageKey::TransactionRolledBack,
            "Транзакция откачена".to_string(),
        );
        messages.insert(
            MessageKey::TransactionAborted,
            "Транзакция прервана".to_string(),
        );
        messages.insert(
            MessageKey::SavepointCreated,
            "Точка сохранения создана".to_string(),
        );
        messages.insert(
            MessageKey::SavepointRolledBack,
            "Откат к точке сохранения".to_string(),
        );

        // Сообщения блокировок
        messages.insert(MessageKey::LockAcquired, "Блокировка получена".to_string());
        messages.insert(
            MessageKey::LockReleased,
            "Блокировка освобождена".to_string(),
        );
        ...
        messages.insert(
            MessageKey::LockDowngraded,
            "Блокировка понижена".to_string(),
        );

        // Сообщения восстановления
        messages.insert(
            MessageKey::RecoveryStarted,
            "Восстановление начато".to_string(),
        );
        messages.insert(
            MessageKey::RecoveryCompleted,
            "Восстановление завершено".to_string(),
        );
        messages.insert(
            MessageKey::CheckpointCreated,
            "Контрольная точка создана".to_string(),
        );
        messages.insert(MessageKey::LogFlushed, "Лог сброшен".to_string());

        // Сообщения производительности
        messages.insert(
            MessageKey::QueryOptimized,
            "Запрос оптимизирован".to_string(),
        );
        ...
        messages.insert(
            MessageKey::StatisticsUpdated,
            "Статистика обновлена".to_string(),
        );
        ...
        messages.insert(
            MessageKey::ProfilingStarted,
            "Профилирование начато".to_string(),
        );
        messages.insert(
            MessageKey::ProfilingStopped,
            "Профилирование остановлено".to_string(),
        );
        messages.insert(MessageKey::TraceStarted, "Трассировка начата".to_string());
        messages.insert(
            MessageKey::TraceCompleted,
            "Трассировка завершена".to_string(),
        );
