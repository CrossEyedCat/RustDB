...
use crate::common::{DatabaseConfig, Language, MessageKey, set_language, t, I18N, I18nManager};
...
impl Cli {
    ...
    pub fn load_config(&self) -> Result<DatabaseConfig, Box<dyn std::error::Error>> {
        let mut config = if let Some(config_path) = &self.config {
            ...
            DatabaseConfig::from_file(&std::path::PathBuf::from("config.toml")).unwrap_or_else(|_| DatabaseConfig::default())
        };
    ...
    pub async fn execute(&self) -> Result<(), Box<dyn std::error::Error>> {
        match &self.command {
            Some(Commands::Server { port, host }) => {
                self.run_server(host.clone(), *port).await
            }
            Some(Commands::Language { action }) => {
                self.handle_language_command(action).await
            }
            Some(Commands::Info) => {
    ...
    async fn handle_language_command(&self, action: &LanguageCommands) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn create_database(&self, name: &str, data_dir: Option<&PathBuf>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn execute_query(&self, query: &str, database: Option<&String>) -> Result<(), Box<dyn std::error::Error>> {
