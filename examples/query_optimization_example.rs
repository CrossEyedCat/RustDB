...
fn demo_statistics_manager() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("     - Размер таблицы: {} байт", table_stats.total_size_bytes);
    println!("     - Количество колонок: {}", table_stats.column_statistics.len());
    
    // Показываем статистику по колонкам
    for (col_name, col_stats) in &table_stats.column_statistics {
        println!("     - Колонка '{}':", col_name);
        println!("       * Уникальных значений: {}", col_stats.distinct_values);
        ...
        match &col_stats.value_distribution {
            ...
            ValueDistribution::Normal { mean, std_dev } => {
                println!("       * Распределение: нормальное (среднее: {}, ст.откл.: {})", mean, std_dev);
            }
            ValueDistribution::Histogram { buckets } => {
                println!("       * Распределение: гистограмма ({} корзин)", buckets.len());
            }
...
fn demo_advanced_optimizer() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("     - Перезапись запросов: {}", optimizer.settings().enable_query_rewriting);
    println!("     - Упрощение выражений: {}", optimizer.settings().enable_expression_simplification);
    println!("     - Вынесение подзапросов: {}", optimizer.settings().enable_subquery_extraction);
    println!("     - Порог стоимости: {}", optimizer.settings().cost_threshold);
...
fn demo_optimization_with_statistics() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("     - Оценка стоимости: {:.2}", test_plan.metadata.estimated_cost);
    println!("     - Оценка количества строк: {}", test_plan.metadata.estimated_rows);
    println!("     - Количество операторов: {}", test_plan.metadata.statistics.operator_count);
    ...
    println!("     - Количество примененных оптимизаций: {}", optimization_result.statistics.optimizations_applied);
    ...
    println!("     - Перезаписи запросов: {}", optimization_result.statistics.query_rewrites);
    println!("     - Упрощения выражений: {}", optimization_result.statistics.expression_simplifications);
    println!("     - Вынесения подзапросов: {}", optimization_result.statistics.subquery_extractions);
    println!("     - Использования статистики: {}", optimization_result.statistics.statistics_usage_count);
    ...
    println!("     - Оценка количества строк: {}", optimization_result.optimized_plan.metadata.estimated_rows);
