...
use rustdb::core::advanced_lock_manager::{AdvancedLockManager, AdvancedLockConfig};
use rustdb::core::concurrency::{ConcurrencyManager, ConcurrencyConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result4 = lock_manager.acquire_lock(
        tx2,
        resource.clone(),
        AdvancedLockMode::Exclusive,
        Some(Duration::from_millis(100))
    ...
    let row_key = RowKey { table_id: 1, row_id: 100 };
    let write_result = concurrency_manager.write(
        tx1,
        row_key.clone(),
        b"test data".to_vec()
    ).await;
    ...
    let read_result = concurrency_manager.read(
        tx2,
        &row_key,
        Timestamp::now()
    ).await;
