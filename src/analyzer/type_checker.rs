...
use crate::analyzer::semantic_analyzer::{TypeInformation, TypeConversion};
...
impl TypeChecker {
    ...
    pub fn check_statement(&mut self, statement: &SqlStatement, _context: &super::AnalysisContext) -> Result<TypeCheckResult> {
    ...
    pub fn check_compatibility(&self, from_type: &DataType, to_type: &DataType) -> TypeCompatibility {
        ...
        if let Some(compatibility) = self.compatibility_rules.get(&(from_type.clone(), to_type.clone())) {
    ...
    pub fn can_convert_implicitly(&self, from_type: &DataType, to_type: &DataType) -> bool {
        ...
        matches!(compatibility, TypeCompatibility::Compatible | TypeCompatibility::CompatibleWithConversion)
    }

    /// Получает результирующий тип для бинарной операции
    pub fn get_binary_operation_result_type(&self, left_type: &DataType, right_type: &DataType, operator: &BinaryOperator) -> Result<DataType> {
        match operator {
            BinaryOperator::Add | BinaryOperator::Subtract | BinaryOperator::Multiply | BinaryOperator::Divide => {
                self.get_arithmetic_result_type(left_type, right_type)
            }
            ...
            BinaryOperator::And | BinaryOperator::Or => {
                ...
                if matches!(left_type, DataType::Boolean) && matches!(right_type, DataType::Boolean) {
                    ...
                    Err(crate::common::Error::semantic_analysis("Logical operators require boolean operands".to_string()))
                }
            }
            _ => {
    ...
    pub fn get_unary_operation_result_type(&self, operand_type: &DataType, operator: &UnaryOperator) -> Result<DataType> {
        match operator {
            UnaryOperator::Plus | UnaryOperator::Minus => {
                match operand_type {
                    DataType::Integer | DataType::Real | DataType::Double => Ok(operand_type.clone()),
                    _ => Err(crate::common::Error::semantic_analysis("Arithmetic unary operators require numeric operands".to_string()))
                }
            }
            UnaryOperator::Not => {
                match operand_type {
                    DataType::Boolean => Ok(DataType::Boolean),
                    _ => Err(crate::common::Error::semantic_analysis("NOT operator requires boolean operand".to_string()))
                }
            }
        }
    }
    ...
    fn check_select_types(&mut self, select: &SelectStatement, result: &mut TypeCheckResult) -> Result<()> {
        ...
        if let Some(where_clause) = &select.where_clause {
            ...
            if !matches!(where_type, DataType::Boolean) {
                result.add_error(TypeCheckError {
                    ...
                    suggested_fix: Some("Use comparison operators to create boolean conditions".to_string()),
                });
            }
        }
        ...
        if let Some(having) = &select.having {
            ...
            if !matches!(having_type, DataType::Boolean) {
                result.add_error(TypeCheckError {
                    ...
                    suggested_fix: Some("Use aggregate functions with comparison operators".to_string()),
                });
            }
        }
    ...
    fn check_insert_types(&mut self, insert: &InsertStatement, result: &mut TypeCheckResult) -> Result<()> {
        result.checks_performed += 1;
    ...
    fn check_update_types(&mut self, update: &UpdateStatement, result: &mut TypeCheckResult) -> Result<()> {
        result.checks_performed += 1;
    ...
    fn check_delete_types(&mut self, delete: &DeleteStatement, result: &mut TypeCheckResult) -> Result<()> {
        result.checks_performed += 1;
    ...
    fn check_create_table_types(&mut self, create: &CreateTableStatement, result: &mut TypeCheckResult) -> Result<()> {
        result.checks_performed += 1;
    ...
    fn check_select_item_types(&mut self, item: &SelectItem, result: &mut TypeCheckResult) -> Result<()> {
        match item {
    ...
    fn check_expression_types(&mut self, expr: &Expression, result: &mut TypeCheckResult) -> Result<DataType> {
        result.checks_performed += 1;
        
        match expr {
            Expression::Literal(literal) => {
                Ok(self.get_literal_type(literal))
            }
            ...
            Expression::BinaryOp { left, op, right } => {
                ...
                if matches!(compatibility, TypeCompatibility::Incompatible) {
                    result.add_error(TypeCheckError {
                        message: format!("Incompatible types in binary operation: {:?} and {:?}", left_type, right_type),
                ...
                if matches!(compatibility, TypeCompatibility::CompatibleWithConversion) {
                    result.add_warning(TypeCheckWarning {
                        message: format!("Implicit type conversion from {:?} to {:?}", right_type, left_type),
                        location: Some("binary expression".to_string()),
    ...
    pub fn get_arithmetic_result_type(&self, left_type: &DataType, right_type: &DataType) -> Result<DataType> {
        match (left_type, right_type) {
            ...
            _ => Err(crate::common::Error::semantic_analysis("Arithmetic operations require numeric operands".to_string()))
        }
    }

    pub fn get_function_result_type(&self, function_name: &str, arg_types: &[DataType], result: &mut TypeCheckResult) -> Result<DataType> {
        match function_name.to_uppercase().as_str() {
            "COUNT" => Ok(DataType::Integer),
            "SUM" => {
                if !arg_types.is_empty() && matches!(arg_types[0], DataType::Integer | DataType::Real | DataType::Double) {
    ...
    fn initialize_rules(&mut self) {
        ...
        // Правила неявных преобразований
        self.conversion_rules.insert(DataType::Integer, vec![DataType::Real, DataType::Text]);
        self.conversion_rules.insert(DataType::Real, vec![DataType::Text]);
        self.conversion_rules.insert(DataType::Boolean, vec![DataType::Text]);
    }
