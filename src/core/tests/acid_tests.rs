...
use crate::core::acid_manager::{AcidManager, AcidConfig, AcidStatistics};
use crate::core::advanced_lock_manager::{AdvancedLockManager, AdvancedLockConfig, ResourceType, LockMode as AdvancedLockMode};
use crate::core::transaction::{TransactionId, IsolationLevel};
    ...
    Fut: std::future::Future<Output = ()>,
{
    tokio::time::timeout(
        ...
        test_fn()
    ).await.expect("Тест превысил лимит времени в 1 секунду");
}
...
async fn create_test_acid_manager() -> AcidManager {
    ...
    let page_manager = Arc::new(PageManager::new(temp_dir, "test_table", page_manager_config).unwrap());
...
async fn test_acid_manager_transaction_lifecycle() {
    run_test_with_timeout(|| async {
        ...
        // Начинаем транзакцию
        assert!(acid_manager.begin_transaction(transaction_id, IsolationLevel::ReadCommitted, false).is_ok());
...
async fn test_acid_manager_read_only_transaction() {
    run_test_with_timeout(|| async {
        ...
        assert!(acid_manager.begin_transaction(transaction_id, IsolationLevel::ReadCommitted, true).is_ok());
...
async fn test_acid_manager_isolation_levels() {
    run_test_with_timeout(|| async {
        ...
        for (i, level) in levels.iter().enumerate() {
            ...
            assert!(acid_manager.begin_transaction(transaction_id, level.clone(), true).is_ok());
...
async fn test_advanced_lock_manager_basic_operations() {
    run_test_with_timeout(|| async {
        ...
        // Получаем блокировку
        assert!(lock_manager.acquire_lock(
            transaction_id,
            resource.clone(),
        ...
        assert!(lock_manager.release_lock(transaction_id, resource.clone()).is_ok());
...
async fn test_advanced_lock_manager_lock_compatibility() {
    run_test_with_timeout(|| async {
        ...
        assert!(lock_manager.release_lock(transaction_id_1, resource.clone()).is_ok());
        assert!(lock_manager.release_lock(transaction_id_2, resource).is_ok());
    }).await;
}
...
async fn test_advanced_lock_manager_deadlock_detection() {
    ...
    run_test_with_timeout(|| async {
        ...
        let result1 = lock_manager.acquire_lock(
            transaction_id_1,
            resource_a.clone(),
            AdvancedLockMode::Exclusive,
            Some(Duration::from_millis(10))
        ...
        let result2 = lock_manager.acquire_lock(
            transaction_id_2,
            resource_b.clone(),
            AdvancedLockMode::Exclusive,
            Some(Duration::from_millis(10))
        ...
        assert!(result1.is_ok(), "Транзакция 1 должна получить блокировку на ресурс A");
        assert!(result2.is_ok(), "Транзакция 2 должна получить блокировку на ресурс B");
...
async fn test_acid_manager_concurrent_transactions() {
    run_test_with_timeout(|| async {
        ...
        for &id in &transaction_ids {
            assert!(acid_manager.begin_transaction(id, IsolationLevel::ReadCommitted, false).is_ok());
        }
